...
def convert_txt_to_sql(input_txt, output_db, table_name="main_table", separator="|", chunk_size=5000, progress_callback=None):
    ...
    try:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if 'ruc' in chunk.columns:
                ...
                chunk['ruc'] = pd.to_numeric(chunk['ruc'], errors='coerce').fillna(0).astype('int64')
        ...
        cursor_idx.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_ruc ON {table_name}(ruc)")
        cursor_idx.close()
